...
def test_raw_photo_to_ideal_pinhole_center():
    cam = Camera(fov=90, res=np.array([100, 100]))
...
def test_poi_to_pinhole_projection_off_axis():
    cam = Camera(fov=120, res=np.array([100, 100]))
    poi = np.array([1, 0, 0])
    ...
    np.testing.assert_allclose(result1[1], result_mag, atol=1e-8)
    ...
    np.testing.assert_allclose(result1[2], result_mag, atol=1e-8)